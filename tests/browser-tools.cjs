...
async function setup(options = {}, launchOptions = {}) {
  ...
  await page.addInitScript(() => {
    ...
    let previous;
    function frame(time) { if (previous && ['play'].includes(window.__castoff?.screen)) window.__audioEvidence.frames.push(time - previous); previous = time; requestAnimationFrame(frame); }
  ...
  return { page, context, browser, errors, failed, requests, consoleErrors, url: process.env.CASTOFF_URL || `http://127.0.0.1:${server.address().port}/castoff-colossus/`, close: async () => { await browser.close(); await new Promise(resolve => server.close(resolve)); } };
}
