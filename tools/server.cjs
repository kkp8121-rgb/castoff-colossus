...
function createServer() {
  return http.createServer((req, res) => {
    let relative;
    try { relative = decodeURIComponent(new URL(req.url, 'http://localhost').pathname).replace(/^\/castoff-colossus(?=\/|$)/, '').replace(/^\/+/, ''); }
...
if (require.main === module) createServer().listen(Number(process.env.PORT) || 4181, '127.0.0.1', () => console.log('SIGHT THIEF local server: http://127.0.0.1:4181/castoff-colossus/'));
